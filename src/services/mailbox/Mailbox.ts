...
export abstract class Mailbox {
    ...
    constructor() {
        ...
        const url = ConfigService.getInstance().api?.wsMailboxUr;
        if (!url) {
            throw Error('No mailbox url');
        }
        this.url = url;
    }
    ...
    public createConnection(cbParams?: ICreateConnectionParams): void {
        ...
        this.socket.onclose = (event: CloseEvent): void => this.onClose(event, cbParams?.onClose);
        this.socket.onerror = (error: Event): void => this.onError(error, cbParams?.onError);
    }
