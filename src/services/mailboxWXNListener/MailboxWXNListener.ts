...
import { generateKeyPair, createIdenticonKey, sharedKey, hash, encrypt, decrypt } from '../../utils';
import {
    IPKMsg,
    TReceivedMsg,
    IRawReceivedMsg,
...
export class MailboxWXNListener {
    ...
    protected _isReady = false; // соединение установлено, произошел обмен публичными ключами и потвержден идентикон
    ...
    protected decryptMessage(message: IRawReceivedMsg): TReceivedMsg {
        switch (message.resp) {
            ...
            case 'success':
                return ({
                    ...message,
                    value: decrypt<ITxDeclinedMsg['value'] | ITxSuccessMsg['value']>({
